...
must_haves:
  truths:
    ...
    - "Each bar type (Essential, Utility) has independent setting"
    - "Setting defaults to false (feature disabled by default)"
    - "Getter/setter functions return correct values"
  artifacts:
    - path: "Core/Defaults.lua"
      provides: "HideWhenOffCooldown default value for Essential and Utility bars"
...
Output: HideWhenOffCooldown field in Defaults.lua for Essential/Utility bars, plus getter/setter functions in Globals.lua for clean API access.
</objective>
...
<task type="auto">
  ...
  <action>
Add `HideWhenOffCooldown = false` to TWO bar types in the CooldownManager section of Defaults.lua:
...
Follow the existing PascalCase convention. Place the new field after the CenterHorizontally field for consistency with similar per-bar boolean settings.

Do NOT add to Buffs, Custom, AdditionalCustom, Item, Trinket, ItemSpell, or BuffBar - those will be handled in future phases if requested.
  </action>
  <verify>
Grep for HideWhenOffCooldown in Defaults.lua - should find exactly 2 occurrences (Essential, Utility).
```bash
grep -c "HideWhenOffCooldown" Core/Defaults.lua
# Expected: 2
grep -n "HideWhenOffCooldown" Core/Defaults.lua
# Expected: Shows lines in Essential and Utility sections
```
  </verify>
  <done>
HideWhenOffCooldown = false appears in Essential and Utility sections of Defaults.lua. AceDB will automatically provide SavedVariables persistence.
  </done>
</task>
